...
target_dist = 0.15; % target is 15 cm ahead of the starting position 
...
% Target position?
stpx_target = targetCtr(1); %stop_point to compute deviation relative to target (and not relative to the endpoint of the movement)
stpy_target = targetCtr(2); %stop_point to compute deviation relative to target(and not relative to the endpoint of the movement)
...
%t_pd50_endpoint = pd_endpoint(20);
%t_pd100_endpoint = pd_endpoint(40);
%t_pd150_endpoint = pd_endpoint(60);
%t_pd200_endpoint = pd_endpoint(80);
%t_pd250_endpoint = pd_endpoint(100);
%t_pd300_endpoint = pd_endpoint(120);
%t_pd350_endpoint = pd_endpoint(140);
%t_pdend_endpoint = pd_endpoint(end);
...
%t_pd50_target = pd_target(20);
%t_pd100_target = pd_target(40);
%t_pd150_target = pd_target(60);
%t_pd200_target = pd_target(80);
%t_pd250_target = pd_target(100);
%t_pd300_target = pd_target(120);
%t_pd350_target = pd_target(140);
%t_pdend_target = pd_target(end);

[maximum_target dummy_target] = max(abs(pd_target));
t_pd_target = pd_target(dummy_target);
t_iadmaxv_target = iad_target(PeakVel_loc); % The initial diviation, at the maximum tangantial velociy
t_iad100_target = iad_target(40); % The initial diviation, 200ms into the movement
t_meanpd_target = mean(pd_target);


% Computing ideal force for a theoretical perfect compensation
%force_x = force(:,1);
%vel_y = MakeVel(pos(:,2),Fs_kin);
%IdealForce_x =  vel_y  .* LateralForceCoefficient;
%MeanLateralForce = mean(force_x);
%MeanIdealLateralForce = mean(IdealForce_x);
%MeanLateralForce_ScaledIdeal = MeanLateralForce ./ MeanIdealLateralForce;
